...
class tx_mydashboard_template
{
    ...
    /* DEPRECATED start */
    public function setDeaultConfig($config)
    {
    ...
    {
        return !($this->widgetDefaultConfig == []);
    }
    ...
    public function getConfigVar($name)
    {
        ...
        } elseif (isset($this->widgetDefaultConfig[$name]['default'])) {
    ...
    {
        if (!is_array($data) || !isset($data['table']) || !isset($data['pageID'])) {
            return '';
        }
    ...
    {
        if (!$this->icon) {
            $this->setIcon(\TYPO3\CMS\Core\Utility\ExtensionManagementUtility::extRelPath('mydashboard') . 'templates/tx_mydashboard_widget_default.png');
        }
        return $this->icon;
    }
}
